...
export class RutasBaseService {
  ...
  async obtenerRutaBasePorNombreBase(nombreBase: Ruta['nombreBase']): Promise<Ruta | null> {
    const col = collection(this.firestore, 'routes');
    const q = query(
      col,
  ...
  async crearMiRutaDesdeBase(baseRutaId: string, nombrePersonalizado: string): Promise<string> {
    ...
    // 2) copiar stops
    const stopsBase = await this.rutasService.obtenerDireccionesOrdenadas(baseRutaId);
